...
DHDesk 是一个薄的 Electron 宿主。它在 Electron main 进程中启动官方 DSH Host，Host 再通过 HTTP/WebSocket Web carrier 提供普通 Web UI；carrier 默认只监听回环地址，也可在用户明确确认风险后向局域网开放。Desktop 没有另造一条 renderer IPC 插件系统，也不把 Electron API 暴露给页面。
...
稳定版与 Beta 是两个实体 npm 包和两个系统应用，不由 Git 分支区分。稳定版使用 `dsh-plugin-desktop`、`DHDesk` 与 `com.zxg.dhdesk`；Beta 使用 `dsh-plugin-desktop-beta`、`DHDesk Beta` 与 `com.zxg.dhdesk.beta`。`upstream.json` 同时记录两个通道的上游版本、提交和 vendored runtime 清单，根级精确 resolution 保证每个 workspace 只能解析自己的 DSH 运行时。

版本检查向官方 GitHub Releases API 发送 `X-DHDesk-Channel: stable|beta` 和当前版本。检查器只接受与通道匹配的 SemVer tag、准确的 DHDesk 平台安装包及其 SHA-256 digest。稳定通道使用 `v<version>` tag，Beta 通道使用 `beta-v<version>` tag 且版本只接受 `-beta.N`；Beta 自动更新只查询 Beta；“安装稳定版”是独立的显式操作，允许选择较低版本并将稳定版安装在 Beta 旁边。

两个通道都必须先发布带 GitHub digest 的完整平台产物，版本才会被检查器发现。否则客户端会把发布数据视为无效，不会静默跨通道下载。
