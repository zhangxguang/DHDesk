/** Isolated command-line environment launched from the DHDesk tray. */
...
/** Platforms with a native terminal launch contract owned by DHDesk. */
...
function macWelcome(
  ...
  return [
    ...
    `printf '%s\\n' ${quoteSh(`DHDesk ${options.productVersion} terminal`)}`,
    ...
    `printf '%s\\n' ${quoteSh('Restart DHDesk after plugin changes.')}`,
...
function windowsWelcome(): string {
  ...
  return [
    ...
    `Write-Host ("DHDesk {0} terminal" -f $env:${WINDOWS_PRODUCT_VERSION})`,
    ...
    `Write-Host 'Restart DHDesk after plugin changes.'`,
    '',
  ].join('\r\n')
}
...
function windowsCmdWelcome(): string {
  ...
  return [
    ...
    `echo(DHDesk !${WINDOWS_PRODUCT_VERSION}! terminal`,
    ...
    `echo(${escapeBatchText('Restart DHDesk after plugin changes.')}`,
...
function windowsLaunchBroker(
  ...
  return [
    '@echo off',
    'setlocal EnableDelayedExpansion',
    `start "DHDesk Beta" /D "!${WINDOWS_PROFILE_DIRECTORY}!" ${target}`,
    'exit /b %errorlevel%',
    '',
  ].join('\r\n')
}
