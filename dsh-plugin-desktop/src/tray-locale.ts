...
const labels: Record<DesktopLocale, Record<DesktopTrayLabelKey, (value: string) => string>> = {
  en: {
    ...
    downloadingUpdate: version => `Downloading DHDesk ${version}…`,
    ...
    updateAvailable: version => `DHDesk ${version} Available`,
  },
  zh: {
    addProfile: () => '新建 Profile…',
    checkForUpdates: () => '检查更新…',
    checkingForUpdates: () => '正在检查更新…',
    downloadingUpdate: version => `正在下载 DHDesk ${version}…`,
    ...
    updateAvailable: version => `DHDesk ${version} 可用`,
  },
}
...
export const rendererRecoveryCopy: Record<DesktopLocale, DesktopRestartConfirmationCopy> = {
  en: {
    title: 'Restore DHDesk',
    message: 'The interface could not recover automatically.',
    detail: 'Automatic recovery stopped after repeated failures to avoid a restart loop. You can try again without restarting the background service. Unsent input may be lost. Export diagnostics from the tray to investigate. Choose Open DHDesk from the tray to return to this prompt later.',
  ...
  zh: {
    title: '恢复 DHDesk',
    message: '界面未能自动恢复。',
    detail: '自动恢复连续失败，为避免重启循环已暂停。可以再次尝试恢复，无需重启后台服务。未发送的输入可能丢失。请从托盘导出诊断信息以继续调查。稍后可从托盘选择“打开 DHDesk”再次打开此提示。',
    confirm: '再次尝试恢复',
    cancel: '暂不处理',
  },
}
...
const restartConfirmationCopy: Record<DesktopLocale, Record<'normal' | 'recovery', DesktopRestartConfirmationCopy>> = {
  en: {
    normal: {
      title: 'Restart DHDesk',
      message: 'Restart DHDesk now?',
    ...
    recovery: {
      ...
      message: 'Restart DHDesk in Recovery Mode?',
