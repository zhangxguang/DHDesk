...
function versionResponse(version: unknown): Response {
  if (typeof version !== 'string') return Response.json({ version })
  const normalized = version.startsWith('v') ? version.slice(1) : version
  const channel = /-beta\.[0-9]+(?:\+.*)?$/u.test(normalized) ? 'beta' : 'stable'
  return releaseResponse(normalized, channel)
}

function releaseResponse(version: string, channel: 'stable' | 'beta'): Response {
  const product = channel === 'beta' ? 'DHDesk-Beta' : 'DHDesk'
  return Response.json([{
    tag_name: channel === 'beta' ? `beta-v${version}` : `v${version}`,
    prerelease: channel === 'beta',
    draft: false,
    assets: [{
      name: `${product}-${version}-universal.dmg`,
      browser_download_url: `https://github.com/zhangxguang/DHDesk/releases/download/${channel === 'beta' ? 'beta-v' : 'v'}${version}/${product}-${version}-universal.dmg`,
...
describe('desktop update Host plugin', () => {
  it('checks Beta automatically and installs an older stable release only through the explicit action', async () => {
    const request = vi.fn(async (_url: string, init: RequestInit) => {
      ...
      return channel === 'beta'
        ? releaseResponse('2.0.6-beta.1', 'beta')
        : releaseResponse('2.0.4', 'stable')
    })
    ...
    expect(harness.downloadAndOpen).toHaveBeenCalledWith(
      ...
      'stable',
      expect.stringContaining('/releases/download/'),
  ...
  it('announces a background update once without opening a confirmation dialog', async () => {
    ...
    await vi.waitFor(() => {
      expect(harness.notifications).toEqual([{
        title: 'DHDesk Update Available',
        body: 'Version 2.1.0 is ready to download. Open DHDesk to continue.',
      }])
    })
    expect(harness.confirmDownload).not.toHaveBeenCalled()
    expect(harness.downloadAndOpen).not.toHaveBeenCalled()
    expect(harness.tray.label()).toBe('DHDesk 2.1.0 Available')
  ...
  it('downloads and opens only after confirmation', async () => {
    ...
    expect(harness.tray.label()).toBe('Downloading DHDesk 2.1.0…')
    expect(harness.notifications).toEqual([])

    resolveDownload()
    await pending
    await vi.waitFor(() => { expect(harness.tray.label()).toBe('DHDesk 2.1.0 Available') })
    expect(harness.notifications).toEqual([])
    expect(harness.tray.label()).toBe('DHDesk 2.1.0 Available')
  })
  ...
  it('rechecks the version after confirmation and skips a rotated download', async () => {
    ...
    expect(harness.downloadAndOpen).not.toHaveBeenCalled()
    expect(harness.showManualCheckResult).not.toHaveBeenCalled()
    expect(harness.tray.label()).toBe('DHDesk 2.2.0 Available')
  })
  ...
  it.each([
    ...
    ['invalid version', async () => Response.json([{ tag_name: 'v2.1.0', prerelease: false, draft: false, assets: [] }]), null],
    ...
    if (expected === null) expect(harness.showManualCheckResult).toHaveBeenCalledWith(null)
    else expect(harness.showManualCheckResult).toHaveBeenCalledWith(expect.objectContaining(expected))
    expect(harness.confirmDownload).not.toHaveBeenCalled()
    expect(harness.downloadAndOpen).not.toHaveBeenCalled()
    expect(harness.notifications).toEqual([])
    expect(harness.warnings).toEqual([])
