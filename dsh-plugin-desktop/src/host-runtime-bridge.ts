...
export function createHostRuntime(rpc: HostRpc, snapshot: RuntimeSnapshot): DesktopRuntime {
  ...
  const runtime: DesktopRuntime = {
    ...
    updates: {
      ...
      downloadAndOpen: (version, signal, channel, artifactUrl, artifactDigest) => send('update:downloadAndOpen', [version, channel, artifactUrl, artifactDigest], signal),
...
export function bindNativeRuntime(rpc: HostRpc, runtime: DesktopRuntime): () => Promise<void> {
  ...
  handle('update:downloadAndOpen', ([version, channel, artifactUrl, artifactDigest], signal) => runtime.updates.downloadAndOpen(version, signal, channel, artifactUrl, artifactDigest))
