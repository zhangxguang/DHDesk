...
describe('final Electron fuse verification', () => {
  it('maps only requested platform and architecture keys to complete runtime contexts', () => {
    const expected = [
      ...
      join('/build', 'mac-universal', 'DHDesk.app', 'Contents', 'MacOS', 'DHDesk'),
      join('/build', 'win-unpacked', 'DHDesk.exe'),
      join('/build', 'win-arm64-unpacked', 'DHDesk.exe'),
    ].sort()
    ...
    expect(resolveFinalPackagedRuntimeContexts(
      ...
      },
      {
        ...
        packager: { appInfo: { productFilename: 'DHDesk' } },
      },
      {
  ...
  it('ignores a stale sibling architecture from an earlier build', () => {
    const expected = join('/build', 'win-unpacked', 'DHDesk.exe')
    const stale = join('/build', 'win-arm64-unpacked', 'DHDesk.exe')
  ...
  it('fails when one requested architecture is missing even if a sibling exists', () => {
    const x64Executable = join('/build', 'win-unpacked', 'DHDesk.exe')

    expect(() => resolveFinalPackagedRuntimeContexts(
      result([{ key: 'win', archs: [Arch.x64, Arch.arm64] }]),
      filename => filename === x64Executable,
    )).toThrow(`win/arm64 at ${join('/build', 'win-arm64-unpacked', 'DHDesk.exe')}`)
  })
  ...
  it('resolves a real target-name map through the target archs retained by NSIS', () => {
    ...
    const built = {
      outDir: '/build',
      configuration: { productName: 'DHDesk' },
  ...
  it('checks every requested final executable after all artifact builds', async () => {
    const executables = [
      join('/build', 'mac-arm64', 'DHDesk.app', 'Contents', 'MacOS', 'DHDesk'),
      join('/build', 'mac', 'DHDesk.app', 'Contents', 'MacOS', 'DHDesk'),
    ]
    const events: string[] = []
  ...
  it.each([
    ...
    await expect(verifyElectronExecutableFuses('/build/DHDesk.exe', read))
  ...
  it('wraps an unreadable final executable with its resolved path', async () => {
    ...
    await expect(verifyElectronExecutableFuses('/build/DHDesk.exe', read))
      .rejects.toThrow('/build/DHDesk.exe')
  })
})
