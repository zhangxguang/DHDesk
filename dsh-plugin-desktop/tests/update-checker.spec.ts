...
function versionResponse(version: unknown, init: ResponseInit = {}): Response {
  ...
  return Response.json([{
    ...
    assets: [{
      ...
      browser_download_url: `https://github.com/zhangxguang/DHDesk/releases/download/v${normalized}/${product}-${normalized}-universal.dmg`,
...
it('selects a versioned DHDesk GitHub Release asset and digest', async () => {
  const result = await checkForStableUpdate({
    ...
    request: async () => Response.json([{
      ...
      draft: false,
      assets: [{
        name: 'DHDesk-2.0.11-universal.dmg',
        browser_download_url: 'https://github.com/zhangxguang/DHDesk/releases/download/v2.0.11/DHDesk-2.0.11-universal.dmg',
        digest: 'sha256:' + 'a'.repeat(64),
      }],
    }]),
  })
  expect(result).toMatchObject({
    ...
    artifactName: 'DHDesk-2.0.11-universal.dmg',
