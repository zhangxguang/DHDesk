/** Headless-safe npm launcher for the DHDesk Electron executable. */
...
Launch DHDesk Beta with the selected Web-capable profile.
...
export function defaultDesktopUserDataDirectory(
  ...
  if (platform === 'win32') {
    ...
    if (appData === undefined || appData.length === 0) {
      throw new Error('APPDATA is unavailable; cannot locate DHDesk diagnostics')
    }
...
async function launchElectron(): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(
      ...
      + 'Or use the packaged DHDesk application.\n',
    )
    return 1
  }
