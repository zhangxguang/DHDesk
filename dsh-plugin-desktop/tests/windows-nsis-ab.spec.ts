  ...
  const options: WindowsNsisAbBuildOptions = {
    ...
    run: (command, args, cwd, env) => {
      ...
      if (args.includes('--dir')) {
        ...
        writeFileSync(join(app, 'DHDesk.exe'), pe())
      ...
      writeFileSync(join(output, 'DHDesk-9.8.7-x64-Setup.exe'), pe())
...
describe('Windows NSIS A/B packaging', () => {
  ...
  it('really reverses only the extract template from below the outer worktree', () => {
    ...
    // The fixture is deliberately below the repository worktree; initialize a
    // tiny index so git can reverse the vendored app-builder patch reliably.
    execFileSync('git', ['-C', isolatedRoot, 'init', '--quiet'])
    execFileSync('git', ['-C', isolatedRoot, 'add', '--', 'templates/nsis/include/extractAppPackage.nsh'])
    ...
    // app-builder-lib may already ship the direct extraction template. When
    // that happens the historical reverse hunk is intentionally a no-op.
    if (restored.includes('CreateDirectory "$PLUGINSDIR\\7z-out"')) {
      ...
      expect(restored).toContain('SetOutPath "$INSTDIR"')
    }
  })
  ...
  it('inspects a readable ASAR and its physical resources', () => {
    ...
    writeFileSync(join(installRoot, 'DHDesk.exe'), pe())
  ...
  it('reports a corrupt app.asar without throwing away the resource identity', () => {
    ...
    writeFileSync(join(root, 'DHDesk.exe'), pe())
  ...
  it('runs the shared packaged-runtime smoke with an Electron Builder-shaped context', () => {
    ...
    expect(contexts[0]).toMatchObject({
      ...
      arch: 1,
      packager: {
        executableName: 'DHDesk',
        appInfo: { productFilename: 'DHDesk' },
      },
    })
