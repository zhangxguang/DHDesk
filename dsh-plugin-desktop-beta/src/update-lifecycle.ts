...
class DesktopUpdateLifecycleOwner implements DesktopUpdateLifecycle {
  ...
  private installStable(): Promise<void> {
    ...
    this.manualTask ??= (async () => {
      ...
      if (result?.status === 'update-available') {
        await this.startDownload(result.latestVersion, 'stable', true, result)
        return
      }
  ...
  private startDownload(
    ...
    channel: DesktopReleaseChannel = this.options.adapter.releaseChannel ?? 'stable',
    allowDowngrade: boolean = false,
    selectedResult?: UpdateCheckResult,
  ): Promise<void> {
    ...
    const task = (async () => {
      ...
      this.registration.refresh()
      try {
        const artifact = confirmedResult?.status === 'update-available' ? confirmedResult : selectedResult
        if (artifact?.artifactUrl === undefined || artifact.artifactDigest === undefined) return
        if (this.options.adapter.releaseChannel === undefined && channel === 'stable') {
          await this.options.adapter.downloadAndOpen(version, controller.signal, undefined, artifact.artifactUrl, artifact.artifactDigest)
        } else {
          await this.options.adapter.downloadAndOpen(version, controller.signal, channel, artifact.artifactUrl, artifact.artifactDigest)
        }
      } catch {
...
function updateAvailableNotification(locale: DesktopLocale, version: string): DesktopNotification {
  return locale === 'zh'
    ? { title: 'DHDesk 有可用更新', body: `版本 ${version} 已可下载。打开 DHDesk 即可继续。` }
    : { title: 'DHDesk Update Available', body: `Version ${version} is ready to download. Open DHDesk to continue.` }
}
