...
function versionResponse(version: unknown, init: ResponseInit = {}): Response {
  if (typeof version !== 'string') return Response.json({ version }, init)
  const normalized = version.startsWith('v') ? version.slice(1) : version
  const channel = /-beta\.[0-9]+(?:\+.*)?$/u.test(normalized) ? 'beta' : 'stable'
  const product = channel === 'beta' ? 'DHDesk-Beta' : 'DHDesk'
  return Response.json([{
    tag_name: channel === 'beta' ? `beta-v${normalized}` : `v${normalized}`,
    prerelease: channel === 'beta',
    draft: false,
    assets: [{
      name: `${product}-${normalized}-universal.dmg`,
      browser_download_url: `https://github.com/zhangxguang/DHDesk/releases/download/${channel === 'beta' ? 'beta-v' : 'v'}${normalized}/${product}-${normalized}-universal.dmg`,
...
function channelVersionResponse(version: unknown, channel: 'stable' | 'beta'): Response {
  if (typeof version !== 'string') return Response.json({ version, channel })
  const product = channel === 'beta' ? 'DHDesk-Beta' : 'DHDesk'
  return Response.json([{
...
it('selects a versioned DHDesk Beta GitHub Release asset and digest', async () => {
  const result = await checkForDesktopUpdate({
    currentVersion: '2.0.11-beta.0',
    channel: 'beta',
    platform: 'darwin',
    request: async () => Response.json([{
      tag_name: 'beta-v2.0.11-beta.1',
      prerelease: true,
      draft: false,
      assets: [{
        name: 'DHDesk-Beta-2.0.11-beta.1-universal.dmg',
        browser_download_url: 'https://github.com/zhangxguang/DHDesk/releases/download/beta-v2.0.11-beta.1/DHDesk-Beta-2.0.11-beta.1-universal.dmg',
        digest: 'sha256:' + 'b'.repeat(64),
      }],
    }]),
  })
  expect(result).toMatchObject({
    status: 'update-available',
    latestVersion: '2.0.11-beta.1',
    artifactName: 'DHDesk-Beta-2.0.11-beta.1-universal.dmg',
