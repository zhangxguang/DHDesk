/** Headless, confirmation-gated downloads for DHDesk installers. */
...
export const DESKTOP_TARGET_VERSION_HEADER = 'X-DHDesk-Target-Version'
...
export interface DownloadDesktopUpdateOptions {
  ...
  /** Browser download URL from the checked GitHub Release asset. */
  readonly downloadUrl?: string
  /** Optional SHA-256 digest supplied by the GitHub Release API. */
...
export async function downloadDesktopUpdate(options: DownloadDesktopUpdateOptions): Promise<string> {
  ...
  const expectedDigest = options.expectedSha256
  const product = channel === 'beta' ? 'DHDesk-Beta' : 'DHDesk'
  const assetName = `${product}-${options.version}-${platform === 'darwin' ? 'universal.dmg' : 'x64-Setup.exe'}`
  if (downloadUrl === undefined || !isDHDeskReleaseAssetUrl(downloadUrl, options.version, assetName, channel)
    || expectedDigest === undefined || !/^[0-9a-f]{64}$/u.test(expectedDigest)) {
    throw new UpdateDownloadError('invalid-options', 'A matching DHDesk release asset and SHA-256 digest are required.')
  }
  ...
  try {
    ...
    await validateSha256(paths.temporary, expectedDigest)
    throwIfAborted(options.signal)
...
async function validateSha256(path: string, expected: string): Promise<void> {
  if (!/^[0-9a-f]{64}$/u.test(expected)) throw new UpdateDownloadError('invalid-options', 'The update digest is invalid.')
  const hash = createHash('sha256')
  for await (const chunk of createReadStream(path)) hash.update(chunk)
  const digest = hash.digest('hex')
  if (digest !== expected) throw new UpdateDownloadError('invalid-artifact', 'The downloaded update digest does not match the release asset.')
}
