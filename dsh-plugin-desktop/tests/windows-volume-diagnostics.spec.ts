...
describe('Windows volume diagnostics', () => {
  ...
  it('accepts fixed NTFS and ReFS volumes', () => {
    expect(diagnoseWindowsVolumes('win32', [{ label: 'install', path: 'C:\\App\\DHDesk.exe' }], query({
      root: 'C:\\',
      fileSystem: 'NTFS',
      driveType: 3,
    }))).toEqual([])
    expect(diagnoseWindowsVolumes('win32', [{ label: 'install', path: 'D:\\App\\DHDesk.exe' }], query({
      root: 'D:\\',
      fileSystem: 'REFS',
      driveType: 3,
    }))).toEqual([])
