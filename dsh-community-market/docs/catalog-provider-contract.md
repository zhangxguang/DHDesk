...
Status: **Implemented public v1 contract.** The versioned Schemas, generated types, strict validation, source persistence, constrained network and media boundaries, standard HTTP adapter, reviewed DSH 1024Store and dshfind adapters, complete local indexing, and loadable Host/Client entries are implemented and tested in DHDesk. This document and its fixtures are the public interoperability contract for `manifestVersion` and `schemaVersion` `1.x`.
...
- A source being bundled as a choice or supported by an adapter does not mean that DHDesk has recommended, audited, or endorsed the source or the plugins it lists.
