...
import {
  ...
  downloadDesktopUpdate as downloadCheckedUpdate,
...
function releaseUrl(platform: DesktopDownloadPlatform, version: string, channel = 'stable'): string {
  const product = channel === 'beta' ? 'DHDesk-Beta' : 'DHDesk'
  return `https://github.com/zhangxguang/DHDesk/releases/download/${channel === 'beta' ? 'beta-v' : 'v'}${version}/${product}-${version}-${platform === 'darwin' ? 'universal.dmg' : 'x64-Setup.exe'}`
}

function downloadDesktopUpdate(options: DownloadDesktopUpdateOptions): Promise<string> {
  return downloadCheckedUpdate({
    downloadUrl: releaseUrl(options.platform, options.version, options.channel),
    expectedSha256: createHash('sha256').update(options.platform === 'darwin' ? dmgArtifact() : windowsArtifact()).digest('hex'),
    ...options,
  })
}
...
describe('desktop update installer download', () => {
  it('pins a Beta artifact request to its channel and target version', async () => {
    ...
    const destination = join(directory, 'DHDesk-Beta-2.0.6-beta.1-mac.dmg')
    ...
    expect(desktopUpdateFilename('darwin', '2.0.6-beta.1', 'beta'))
      .toBe('DHDesk-Beta-2.0.6-beta.1-mac.dmg')
  })
  ...
  it('accepts a Beta artifact without response identity headers', async () => {
    ...
    const result = await downloadDesktopUpdate({
      ...
      version: '2.0.6-beta.1',
      channel: 'beta',
      destinationPath: join(directory, 'DHDesk-Beta-2.0.6-beta.1-mac.dmg'),
      request: async () => chunkedResponse([dmgArtifact()]),
    })
    expect(result).toBe(join(directory, 'DHDesk-Beta-2.0.6-beta.1-mac.dmg'))
  })
  ...
  it('streams a macOS DMG from only the fixed endpoint and atomically completes it', async () => {
    ...
    expect(result).toBe(join(directory, 'DHDesk-2.1.0-mac.dmg'))
    expect(await readFile(result)).toEqual(Buffer.from(artifact))
    expect(calls).toHaveLength(1)
    expect(calls[0]?.url).toBe(releaseUrl('darwin', '2.1.0'))
  ...
  it('accepts a Windows executable only when it has both MZ and PE signatures', async () => {
    ...
    const result = await downloadDesktopUpdate({
      ...
      request: async (url) => {
        expect(url).toBe(releaseUrl('win32', '2.2.0'))
        return chunkedResponse([artifact])
      },
    })

    expect(result).toBe(join(directory, 'DHDesk-2.2.0-windows.exe'))
    expect(await readFile(result)).toEqual(Buffer.from(artifact))
    await expectNoPartialFiles(directory)
  })
