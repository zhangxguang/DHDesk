...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `DHDesk-Beta-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'DHDesk Beta.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing DHDesk Beta.exe: ${portablePath}`)
  }
