...
const COPY: Record<DesktopLocale, DesktopRecoveryCopy> = {
  en: {
    title: 'DHDesk Recovery Assistant',
    fallbackBody: 'The recovery information could not be read. Quit and start DHDesk again.',
    ...
    confirmRestoreDefaultDirectoryBody: 'DHDesk will use the default data directory for this system after restart. The current data directory will not be deleted.',
    ...
    confirmCreateDefaultDirectoryBody: 'DHDesk will create a new environment at the default path and use it after restart. The current data directory will not be deleted.',
    confirmCreateDefaultDirectoryAction: 'Create and restart',
    confirmFactoryReset: 'Factory reset DHDesk?',
    confirmFactoryResetMessage: 'Remove current DSH data and initialize a new environment?',
    confirmFactoryResetBody: currentDirectory => `The following directory will be moved to the operating-system Trash or Recycle Bin:\n\n${currentDirectory}\n\nProfiles, plugins, settings, credentials, sessions, attachments, and workspace records stored there will be removed from DHDesk. Project files outside this directory are not deleted. DHDesk will restart and create a clean default Profile.`,
    ...
    confirmRollbackBody: capturedAt => `This immediately restores the current Profile plus the checkpointed settings.yaml and Harness-home patch captured at ${capturedAt}. After restarting, DHDesk will use the rolled-back configuration.`,
    confirmRollbackAction: 'Restore configuration',
    uninstalledSuccess: 'The plugin was removed from the current Profile. Restart DHDesk to use the updated plugin configuration.',
    rollbackSuccess: slotId => `Rolled back to ${slotId}. Restart DHDesk to use this configuration; the first healthy start after rollback will preserve all three existing slots.`,
    profileSelectedSuccess: 'This Profile is now selected. Restart DHDesk to use it.',
  ...
  zh: {
    title: 'DHDesk 恢复助手',
    fallbackBody: '无法读取恢复信息。请退出并重新启动 DHDesk。',
    reason: '进入恢复模式的原因',
    ...
    confirmRestoreDefaultDirectoryBody: 'DHDesk 将改为使用当前系统的默认数据目录。当前数据目录不会被删除。',
    ...
    confirmCreateDefaultDirectoryBody: 'DHDesk 将在默认路径创建一个全新的环境并重启。当前数据目录不会被删除。',
    confirmCreateDefaultDirectoryAction: '新建并重启',
    confirmFactoryReset: '恢复 DHDesk 出厂设置？',
    confirmFactoryResetMessage: '移除当前 DSH 数据并重新初始化？',
    confirmFactoryResetBody: currentDirectory => `以下目录将被移入系统废纸篓或回收站：\n\n${currentDirectory}\n\n其中保存的 Profile、插件、设置、凭据、会话、附件和工作区记录都会从 DHDesk 中移除；不会删除此目录以外的项目文件。随后 DHDesk 会重启并创建干净的默认 Profile。`,
    ...
    confirmRollbackBody: capturedAt => `将立即恢复 ${capturedAt} 创建的检查点中的 Profile、共享设置文件 settings.yaml 与 DSH 数据目录补丁；重启后，DHDesk 将使用回滚后的配置。`,
    confirmRollbackAction: '恢复配置',
    uninstalledSuccess: '插件已从当前 Profile 中卸载。请重启 DHDesk 以使用更新后的插件配置。',
    rollbackSuccess: slotId => `已恢复检查点 ${slotId}。重启后将使用恢复的配置；首次成功启动时会保留现有检查点。`,
    profileSelectedSuccess: '已设为当前 Profile。请重启 DHDesk 以使用该 Profile。',
