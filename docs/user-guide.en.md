...
Plugins are extensions that add capabilities to DSH, such as models, tools, interfaces, and workflows. DHDesk uses the same plugin system as official Harness, so official plugins install and work directly; multiple plugins follow the same conventions and can be installed and used together.
...
In the terminal opened from the DHDesk tray, bare `dsh` and plugin commands without `--profile` default to the active profile:
...
An explicit `--profile <name>` always wins. Restart DHDesk after plugin changes so the new bundle enters the Loader composition.
...
Packaged macOS and Windows applications check `https://api.github.com/repos/zhangxguang/DHDesk/releases` in the background. Startup is not blocked; network errors, non-200 responses, invalid versions, and a server version that is not newer remain silent in the background. A newer version updates the tray and raises one non-blocking system notification per version instead of opening a download confirmation automatically; clicking the notification reveals Desktop.
...
- **The application crashes repeatedly before the tray appears**: run the installed executable directly with the recovery option. The default Windows installation command is below; replace the path if you selected another installation directory.
  ...
  ```powershell
  & "$env:LOCALAPPDATA\Programs\DHDesk\DHDesk.exe" --export-diagnostics
  ```
