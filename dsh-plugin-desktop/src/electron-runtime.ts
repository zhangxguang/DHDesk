...
/** Native adapter used by the DHDesk launcher and owned by its Cordis shell plugin. */
export class ElectronDesktopRuntime implements DesktopRuntime {
  ...
  constructor(
    ...
    this.updates = {
      ...
      downloadAndOpen: (version, signal, channel, artifactUrl, artifactDigest) => this.downloadAndOpenUpdate(version, signal, channel, artifactUrl, artifactDigest),
  ...
  private async downloadAndOpenUpdate(
    ...
    const artifactPath = await downloadDesktopUpdate({
      ...
      destinationPath,
      ...(artifactUrl === undefined ? {} : { downloadUrl: artifactUrl }),
      ...(artifactDigest === undefined ? {} : { expectedSha256: artifactDigest }),
      request: (url, init) => net.fetch(url, init),
      signal,
    })
    signal.throwIfAborted()
