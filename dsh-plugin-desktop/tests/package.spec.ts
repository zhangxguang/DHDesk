...
const releaseWorkflow = readFileSync(new URL('.github/workflows/release.yml', workspaceRoot), 'utf8')
const macReleaseScript = readFileSync(new URL('scripts/release-mac.ts', packageRoot), 'utf8')
const betaMacReleaseScript = readFileSync(new URL('../dsh-plugin-desktop-beta/scripts/release-mac.ts', packageRoot), 'utf8')
...
describe('published package surface', () => {
  ...
  it('fixes the installed application identity', () => {
    ...
    expect(manifest.build?.productName).toBe('DHDesk')
    expect(manifest.build?.appId).toBe('com.zxg.dhdesk')
    ...
    expect(manifest.build?.win?.artifactName).toBe('DHDesk-${version}-${arch}-Portable.${ext}')
    expect(manifest.build?.nsis).toEqual({
      ...
      useZip: false,
      artifactName: 'DHDesk-${version}-${arch}-Setup.${ext}',
    })
  ...
  it('keeps the release workflow single-publisher and blocks unsigned Windows assets', () => {
    expect(ciWorkflow).not.toContain('Temporarily skip AA profile smoke')
    expect(ciWorkflow).not.toContain('writeFileSync(path')
    expect(macReleaseScript).toContain("'--publish', 'never'")
    expect(betaMacReleaseScript).toContain("'--publish', 'never'")
    expect(releaseWorkflow).toContain('Enforce Windows Authenticode release gate')
    expect(releaseWorkflow).toContain("grep -Fqx 'Authenticode: verified'")
    expect(releaseWorkflow).toContain("grep -Fqx 'Release-Publishable: true'")
  })
