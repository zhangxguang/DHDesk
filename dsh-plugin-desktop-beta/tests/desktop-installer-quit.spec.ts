...
describe('Desktop installer quit request', () => {
  it('accepts only the dedicated flag on Windows', () => {
    ...
    expect(isDesktopInstallerQuitRequest(
      ['DHDesk.exe', DESKTOP_INSTALLER_QUIT_FLAG],
      'win32',
    )).toBe(true)
    expect(isDesktopInstallerQuitRequest(['DHDesk.exe', '--quit'], 'win32')).toBe(false)
    expect(isDesktopInstallerQuitRequest(
      ['DHDesk', DESKTOP_INSTALLER_QUIT_FLAG],
      'darwin',
    )).toBe(false)
  })

  it('distinguishes background Node re-entry from an explicit application launch', () => {
    expect(isDesktopBackgroundNodeRequest(['DHDesk.exe'])).toBe(false)
    expect(isDesktopBackgroundNodeRequest(['DHDesk.exe', '--profile', 'desktop'])).toBe(false)
    expect(isDesktopBackgroundNodeRequest(['DHDesk.exe', 'C:\\app\\pnpm\\bin\\pnpm.mjs', 'install'])).toBe(true)
    expect(isDesktopBackgroundNodeRequest(['DHDesk.exe', '--require', 'C:\\runtime\\clear-env.cjs'])).toBe(true)
    expect(isDesktopBackgroundNodeRequest(['DHDesk.exe', '--import=file:///runtime/clear-env.mjs'])).toBe(true)
    expect(isDesktopBackgroundNodeRequest(['DHDesk.exe', '--expose-internals', 'desktop-cli.js'])).toBe(true)
  })
