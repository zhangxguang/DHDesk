...
describe('final Electron fuse verification', () => {
  it('maps only requested platform and architecture keys to complete runtime contexts', () => {
    const expected = [
      ...
      join('/build', 'mac-universal', 'DHDesk Beta.app', 'Contents', 'MacOS', 'DHDesk Beta'),
      join('/build', 'win-unpacked', 'DHDesk Beta.exe'),
      join('/build', 'win-arm64-unpacked', 'DHDesk Beta.exe'),
    ].sort()
    ...
    expect(resolveFinalPackagedRuntimeContexts(
    ...
    )).toEqual([
      {
        ...
        packager: {
          ...
          appInfo: { productFilename: 'DHDesk Beta' },
        },
      },
      {
        ...
        packager: { appInfo: { productFilename: 'DHDesk Beta' } },
      },
      {
  ...
  it('ignores a stale sibling architecture from an earlier build', () => {
    const expected = join('/build', 'win-unpacked', 'DHDesk Beta.exe')
    const stale = join('/build', 'win-arm64-unpacked', 'DHDesk Beta.exe')
  ...
  it('fails when one requested architecture is missing even if a sibling exists', () => {
    const x64Executable = join('/build', 'win-unpacked', 'DHDesk Beta.exe')

    expect(() => resolveFinalPackagedRuntimeContexts(
      result([{ key: 'win', archs: [Arch.x64, Arch.arm64] }]),
      filename => filename === x64Executable,
    )).toThrow('win/arm64 at /build/win-arm64-unpacked/DHDesk Beta.exe')
  })
  ...
  it('resolves a real target-name map through the target archs retained by NSIS', () => {
    ...
    const built = {
      outDir: '/build',
      configuration: { productName: 'DHDesk Beta' },
  ...
  it('checks every requested final executable after all artifact builds', async () => {
    const executables = [
      join('/build', 'mac-arm64', 'DHDesk Beta.app', 'Contents', 'MacOS', 'DHDesk Beta'),
      join('/build', 'mac', 'DHDesk Beta.app', 'Contents', 'MacOS', 'DHDesk Beta'),
    ]
    const events: string[] = []
  ...
  it.each([
    ...
    await expect(verifyElectronExecutableFuses('/build/DHDesk Beta.exe', read))
  ...
  it('wraps an unreadable final executable with its resolved path', async () => {
    ...
    await expect(verifyElectronExecutableFuses('/build/DHDesk Beta.exe', read))
      .rejects.toThrow('/build/DHDesk Beta.exe')
  })
})
