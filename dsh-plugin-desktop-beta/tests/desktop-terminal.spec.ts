...
function macOptions(stateDir: string, spawn: DesktopTerminalSpawn): DesktopTerminalOptions {
  return {
    platform: 'darwin',
    appExecutable: "/Applications/DSH O'Brien.app/Contents/MacOS/DHDesk",
...
function windowsOptions(stateDir: string, spawn: DesktopTerminalSpawn): DesktopTerminalOptions {
  return {
    platform: 'win32',
    appExecutable: 'C:\\Program Files\\DSH 100% Desktop\\DHDesk.exe',
    dshBootstrapPath: 'C:\\Program Files\\DHDesk\\resources\\app.asar\\lib\\dsh-terminal-bootstrap.js',
    pnpmBinPath: 'C:\\Program Files\\DHDesk\\resources\\app.asar\\node_modules\\pnpm\\bin\\pnpm.mjs',
    electronVersion: '43.4.0',
    profileName: 'desktop',
    productVersion: '2.0.0',
...
describe('desktop terminal environment', () => {
  ...
  it('generates private macOS shims and opens one quoted welcome command', () => {
    ...
    expect(dshShim).toContain("'/Applications/DSH O'\"'\"'Brien.app/Contents/MacOS/DHDesk'")
    ...
    expect(nodeShim).toBe([
      '#!/bin/sh',
      `ELECTRON_RUN_AS_NODE=1 exec '/Applications/DSH O'"'"'Brien.app/Contents/MacOS/DHDesk' "$@"`,
      '',
    ].join('\n'))
    ...
    expect(welcome).toContain('DHDesk 2.0.0 terminal')
    ...
    expect(welcome).toContain('Restart DHDesk after plugin changes.')
  ...
  it('generates Windows batch shims and opens PowerShell through a visible-console broker', () => {
    ...
    expect(welcome).toContain('"DHDesk {0} terminal" -f $env:DSH_DESKTOP_PRODUCT_VERSION')
    ...
    expect(launcher).toContain('start "DHDesk Beta" /D "!DSH_DESKTOP_PROFILE_DIRECTORY!"')
  ...
  it('keeps Windows scripts ASCII and passes localized paths through the child environment', () => {
    ...
    options.appExecutable = 'C:\\程序\\DHDesk.exe'
    ...
    expect(harness.calls[0]?.options.env).toEqual(expect.objectContaining({
      ...
      DSH_DESKTOP_APP_EXECUTABLE: 'C:\\程序\\DHDesk.exe',
