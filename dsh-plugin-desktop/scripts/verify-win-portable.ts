...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `DHDesk-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'DHDesk.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing DHDesk.exe: ${portablePath}`)
  }
