...
describe('published package surface', () => {
  ...
  it('sets a distinct Beta process identity before taking the single-instance lock', () => {
    ...
    expect(productIdentity).toContain("productName: 'DHDesk Beta'")
    expect(productIdentity).toContain("appId: 'com.zxg.dhdesk.beta'")
  ...
  it('fixes the installed application identity', () => {
    ...
    expect(manifest.bin).not.toHaveProperty('dsh-desktop')
    expect(manifest.bin).not.toHaveProperty('dsh-plugin-desktop')
    expect(manifest.build?.productName).toBe('DHDesk Beta')
    expect(manifest.build?.appId).toBe('com.zxg.dhdesk.beta')
    ...
    expect(manifest.build?.mac?.artifactName).toBe('DHDesk-Beta-${version}-${arch}.${ext}')
    ...
    expect(manifest.build?.win?.artifactName).toBe('DHDesk-Beta-${version}-${arch}-Portable.${ext}')
    expect(manifest.build?.nsis).toEqual({
      ...
      shortcutName: 'DHDesk Beta',
      useZip: false,
      artifactName: 'DHDesk-Beta-${version}-${arch}-Setup.${ext}',
    })
