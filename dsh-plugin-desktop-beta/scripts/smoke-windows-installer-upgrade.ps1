...
$taskExistingInstalls = @(Get-ItemProperty $taskUninstallRoots -ErrorAction SilentlyContinue | Where-Object {
  $_.DisplayName -match '^DHDesk'
})

if ($taskExistingProcesses.Count -gt 0 -or $taskExistingInstalls.Count -gt 0) {
  throw 'Refusing to run while an existing DHDesk process or installation is present.'
}
...
$taskUserData = Join-Path $env:APPDATA 'DHDesk'
$taskDshHome = Join-Path $taskRoot 'dsh-home'
$taskActiveRunMarker = Join-Path $taskUserData 'crash-evidence\active-run.json'
$taskAppPath = Join-Path $taskInstallRoot 'DHDesk Beta.exe'
$taskUninstallerPath = Join-Path $taskInstallRoot 'Uninstall DHDesk Beta.exe'
if (Test-Path -LiteralPath $taskActiveRunMarker) {
  throw 'Refusing to overwrite an existing DHDesk active run marker.'
}
$taskResult = [ordered]@{
...
function Wait-TaskProcess([bool]$taskShouldExist, [int]$taskTimeoutSeconds = 30) {
  ...
  throw "Timed out waiting for DHDesk process state: shouldExist=$taskShouldExist"
}
...
try {
  ...
  $taskRemainingShortcuts = @($taskShortcutRoots | ForEach-Object {
    Get-ChildItem -LiteralPath $_ -Filter '*DHDesk*' -Recurse -ErrorAction SilentlyContinue
  })
