...
In this policy, “we” means the **DHDesk project maintainer team** that maintains and publishes the official DHDesk distribution from the [`zhangxguang/DHDesk`](https://github.com/zhangxguang/DHDesk) GitHub repository. DHDesk is an independent community project and has no affiliation, partnership, authorization, or endorsement relationship with DeepSeek.
...
- DHDesk provided through the [official GitHub repository](https://github.com/zhangxguang/DHDesk) and official release channels;
- the official website, version-check service, and download redirects under `https://github.com/zhangxguang/DHDesk/`; and
- privacy requests, support email, or issue reports that you voluntarily send to the project maintainers.

This policy does not control processing performed by third-party forks, modified builds, third-party distributors, model providers, plugins, marketplace sources, or package services. It applies only to the official DHDesk distribution and the official services described here.

## 2. Summary

- DHDesk profiles, settings, workspaces, sessions, logs, and crash files remain on your device by default.
- The official update service does not require a DHDesk account. Its version-check code does not intentionally send prompts, responses, file contents, workspace paths, profile names, session contents, API keys, MAC addresses, or hardware serial numbers.
...
```text
GET https://api.github.com/repos/zhangxguang/DHDesk/releases
```
...
- `X-DHDesk-Installation-Id: <random UUID v4>`.
- `X-DHDesk-Version: <canonical installed stable version>`.
...
- macOS: `~/Library/Application Support/DHDesk/identity/installation-id`;
- Windows: `%APPDATA%\DHDesk\identity\installation-id`.
...
We do not use the installation UUID for advertising profiles or cross-service tracking, and we do not sell it. A version check requires no login, and the client sends no name, email address, or DHDesk account ID in that request.
...
The following information remains on your device by default rather than being uploaded automatically to DHDesk:
...
DHDesk is a composable plugin platform. The following transfers are triggered by services, sources, plugins, or actions you choose. Each recipient processes information under its own privacy terms.
...
Allowing DSH to open in a browser does not itself upload a session to DHDesk. It lets an ordinary browser access the local Host and is available only in compatibility mode. Loopback access is limited to `127.0.0.1` by default.
...
LAN traffic normally does not pass through DHDesk, but a person who connects to your computer over the LAN becomes a recipient of the data exposed through that connection.
...
DHDesk is a tool for developers and people able to manage a local computing environment and is not directed specifically to children. Minors should use it with a guardian's guidance. If you believe we processed a child's personal data without satisfying applicable requirements, contact us so that we can investigate and take appropriate action.
