...
`dsh-plugin-desktop` runs DSH in Electron while remaining part of the ordinary Cordis composition. The installed application is named **DHDesk**. The package provides the `dsh-plugin-desktop` executable and the `dsh-desktop` alias; the registered npm package name is the reliable `npx` entry.
...
Packaged macOS and Windows applications query `https://api.github.com/repos/zhangxguang/DHDesk/releases` 60 seconds after startup and every six hours after a completed check. Each no-cache request has a 15-second deadline, sends `X-DHDesk-Channel: stable` together with the installed version, and shares one in-flight operation with the **Check for Updates…** tray command. Stable accepts only canonical release SemVer entries with an exact platform asset and SHA-256 digest; Beta releases are ignored. Background failures and non-newer versions remain silent; a manual check always opens a native result dialog. Development, unpackaged, and Linux launches do not download an installer.
...
Release operators must publish both platform artifacts and their GitHub asset digests before making a stable version discoverable. The checker selects only exact stable tags and DHDesk platform assets from the official repository. Missing, unavailable, mismatched, or invalid release data produces no Desktop prompt.
...
DHDesk writes UTF-8 logs under Electron's user-data directory: `%APPDATA%\DHDesk\logs` on Windows and `~/Library/Application Support/DHDesk/logs` on macOS. Full logs use `dsh-YYYY-MM-DD.log`; warnings and errors are also written to `dsh-YYYY-MM-DD.error.log`. Files rotate at 10 MiB, files older than seven days are removed at startup, and the directory is kept below 200 MiB. The `dsh-desktop.logLevel` setting controls verbosity and defaults to `info`.
