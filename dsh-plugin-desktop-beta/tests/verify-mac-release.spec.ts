...
function options(overrides: Partial<MacReleaseVerificationOptions> = {}) {
  ...
  const value: MacReleaseVerificationOptions = {
    ...
    listDmgs: () => ['/release/dist/DHDesk-Beta-2.0.0-universal.dmg'],
...
describe('macOS release artifact verification', () => {
  it('mounts one DMG and verifies signature, Gatekeeper, and the stapled ticket', () => {
    ...
    const appPath = join('/private/tmp/dsh-desktop-dmg-test', 'DHDesk Beta.app')

    expect(verifyMacRelease(harness.value)).toEqual({
      appPath,
      dmgPath: '/release/dist/DHDesk-Beta-2.0.0-universal.dmg',
    })
    ...
    expect(harness.calls).toEqual([
      {
        command: 'hdiutil',
        args: [
          'attach', '/release/dist/DHDesk-Beta-2.0.0-universal.dmg',
          '-mountpoint', '/private/tmp/dsh-desktop-dmg-test', '-nobrowse', '-readonly',
        ],
      },
      {
        command: 'lipo',
        args: [join(appPath, 'Contents', 'MacOS', 'DHDesk Beta'), '-verify_arch', 'x86_64'],
      },
      {
        command: 'lipo',
        args: [join(appPath, 'Contents', 'MacOS', 'DHDesk Beta'), '-verify_arch', 'arm64'],
      },
